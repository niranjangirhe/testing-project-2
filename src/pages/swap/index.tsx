...
const Recent = () => {
  ...
  const swapnow = async () => {
    if (swapTopPutValue <= Number(swapTokenTop.data)) {
      ...
      const transferExtrinsic = api.tx.exchange.swapExactTokensForTokens(
        token_a_real,
        0,
        [swapTokenTop.tokenId, swapTokenTail.tokenId],
        intactWalletAddress,
        next_block,
      );
    ...
    <>
      ...
      <div className="bg-W3GBG  p-3 rounded-2xl">
        <div className="flex justify-between">
          ...
          <div className="flex">
            ...
            <div className="px-1.5 pt-1.5 text-center text-gray-200 text-sm bg-W3GInfoBG rounded-full ">Wallet</div>
            {/*<div>*/}
        ...
        <div className="flex justify-between mt-5">
          ...
          <div className="text-sm mt-2 flex ml-1 text-gray-400">
            Balance: <div className="">{swapTokenTop.data}</div>
          </div>
        </div>
      </div>
      ...
      <div className="text-center mt-5  text-white">
        ...
        <div className={WalletButtonShow ? 'mt-1' : 'hidden'}>
          <div className={classNames(requestData ? 'hidden' : '')}>
            <button
              className={classNames(
                swapTopPutValue > Number(swapTokenTop.data)
                  ? ' cursor-not-allowed from-[#DB5E7F]/60  via-[#876BD2]/60 to-[#6E93E8]/60 w-64 py-1.5 text-gray-400 rounded-lg text-xl font-semibold bg-gradient-to-r'
                  : 'hidden',
              )}
            >
              Swap
            ...
            <button
              ...
              className={classNames(
                swapTopPutValue > Number(swapTokenTop.data)
                  ? 'hidden'
                  : 'from-[#DB5E7F]  via-[#876BD2] to-[#6E93E8] w-64 py-1.5  rounded-lg text-xl font-semibold bg-gradient-to-r',
              )}
            >
              Swap
            </button>
          ...
          <button
            className={classNames(
              requestData ? 'from-[#DB5E7F]/60  via-[#876BD2]/60 to-[#6E93E8]/60 cursor-not-allowed' : 'hidden',
              'w-64 py-1.5 rounded-lg text-xl font-semibold bg-gradient-to-r',
            )}
          >
    ...
    <>
      <div className="bg-gray-900  bg-opacity-70 p-3 rounded-2xl">
        <div className="flex justify-between">
          ...
          <div className="flex">
            <div className="text-gray-300 mt-2 text-xs mr-2">Pay from</div>
            <div className="px-1.5 pt-1.5 text-center text-gray-200 text-sm bg-gray-700 rounded-full ">Wallet</div>
            <div>
      ...
      <div className="flex gap-3">
        <div className="flex justify-between">
          <div className="flex flex-col gap-1 mt-2">
            <div className="text-sm leading-5  font-medium px-2">Rate</div>
            <div className="flex justify-between items-baseline bg-dark-900 rounded px-4 py-1.5 border border-dark-700 hover:border-dark-600">
              <div className="text-lg leading-6 font-bold relative flex items-baseline flex-grow gap-3 overflow-hidden">
                ...
                />
                <div className="text-sm leading-5 font-medium cursor-pointer select-none currentColor">W3G</div>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1 mt-2">
            <div className="text-sm leading-5  font-medium px-2">Expires in</div>
            <div className="flex justify-between items-baseline bg-dark-900 rounded px-4 py-1.5 border border-dark-700 hover:border-dark-600">
              <div className="text-lg leading-6 font-bold relative flex items-baseline flex-grow gap-3 overflow-hidden">
      ...
      <div className="text-center mt-5  text-black">
        ...
        <div className={WalletButtonShow ? 'mt-1' : 'hidden'}>
          <button onClick={() => {}} className="px-24 py-1.5 rounded-lg bg-indigo-400">
            Swap
          </button>
        </div>
      </div>
  ...
  return (
    <div>
          ...
          <div className="bg-black  px-5 py-7 rounded-xl">
            <Tab.Group>
              ...
              <Tab.Panels className="mt-2 ">
                <Tab.Panel className={classNames(' rounded-xl p-1  md:w-97 ')}>
                  <Recent />
                </Tab.Panel>
                {/*Popular*/}
                <Tab.Panel className={classNames('text-gray-300 rounded-xl p-1 md:w-97')}>{/* <Popular/> */}</Tab.Panel>
              </Tab.Panels>
            </Tab.Group>
          </div>
        </div>
      ...
      <div className="text-gray-600 text-sm flex justify-center text-center mt-2 md:w-1/2 mx-auto">
        Tip: When expert mode is enabled, balance isn`t checked when creating orders. You can use this to chain limit
        orders.
      </div>
      <SelectTokenTop />
      <SelectTokenTail />
